...
module.exports = {
  ...
  getAllRecordings: async (req, res, next) => {
    ...
    try {
      ...
      let recordedStreams = await liveStreamServices.getRecordedStreams(custId, req?.query?.from, req?.query?.to, req?.query?.location, req?.query?.zones,req.query?.live,req.query?.vod, req.query?.sortBy, pageNumber, pageSize, pageCount, t);
